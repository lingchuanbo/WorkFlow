...
- 主程序
    - 调整为更合理的目录结构。
    - 删除默认的配置文件config.ini，运行时若无此文件，自动复制config.ini.help.txt为此文件。
    - 支持用户自定义功能标签，在config.ini所在目录新增custom.ahk，则会自动包含进来，帮助见config.ini.help.txt。
...
--------
- 主程序
    - `WorkFlow.ahk`、`plugins/General/General.ahk` 和 `lib` 下部分文件改用 linxinhong 最新版本的。
