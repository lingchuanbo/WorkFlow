// 创建窗口
var win = new Window('palette', '2层循环 ByBoBO');
this.windowRef = win;

win.pnl = win.add("panel", [5, 5, 170, 80], '控制');
...
function _Loop2(){
    ...
    var nOpacity = [0, compDuration, compDuration * 2]; //帧所在位置
    
    var nValue = [0, 100, 0]; //透明K键帧数组
...
function _splitComp() {
  ...
  function duplicateMarker(ac) {
    ...
    for (var Z = 0; Z < aa.length; Z += 1) {
      ...
      var strTime = [0, compDuration * 1, compDuration * 2];
  ...
  function _addCompose() {
    ...
    if (actComp instanceof CompItem) {
      ...
      if (myLayers.length > 0) {
        ...
        layerIndices.push(layerIndices - 1, layerIndices - 2);
